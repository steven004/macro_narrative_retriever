...
def filter_news_with_llm(news_list):
    ...
    prompt = """
    你是一个极其严苛的宏观对冲基金数据审核员。你的唯一任务是剔除所有的【个股与微观噪音】。

    【绝对红线（只要触发一条，立刻判定 is_macro: false，且 importance_score 为 0）】：
    1. 新闻的主语是某一家具体的上市公司（例如：Apple, NVIDIA, PPG Industries, KeyCorp, P&G 等）。
    2. 新闻讨论了特定公司的股票价格、评级下调/上调、财报、估值或分析师目标价。
    3. 新闻标题带有股票代码（如 NYSE:ET, NASDAQ:NEXT 等）。
    注意：即使这篇个股新闻中提到了“通胀”、“战争”或“美联储”等宏观背景，只要它的核心是在分析某家公司的业务或股票，它就是【伪宏观】，必须被严格剔除！

    【只有符合以下条件，才能判定 is_macro: true】：
    新闻的主语必须是国家经济、主权货币、大宗商品全局（原油、黄金总量）、央行政策（美联储、欧央行）或重大的地缘政治全局影响。

    对所有判定为 is_macro: true 的新闻提供一个 1-10 的整数重要性评分 (importance_score)。10分代表具有全球系统性影响的顶级宏观事件。
