...
def filter_news_with_llm(news_list, chunk_size=50):
    ...
    【极致重要：事件去重与领域多样性（同质化降权）】：
    在当前周期内，地缘政治冲突（如中东战争）会产生极度密集、碎片化的滚动新闻。为了防止最终高分列表被同一个事件“霸榜”，请你严格执行去重逻辑：
    1. 给同一个持续性事件中的【琐碎跟进与重复抗议】强制打低分（1-4分）。
    2. 只允许给该领域中**真正具有转折意义**的 1-2 条新闻打高分。

    【重要：输出多样性与中国市场权重】：
    当处理全球宏观新闻时，请优先考虑那些对中国市场（A股、汇率、外贸）有外溢影响的事件。
    如果你在处理的是中文源（如第一财经、财联社等），请确保中国本土政策层面的新闻占据足够比重。
    如果全都是海外新闻且无直接中国相关内容，请依然按重要性打分，**绝对不要因为缺乏中国新闻就完全拒绝输出全球宏观事件**。

    【极其重要的输出格式要求（为了节省Token）】：
    请**只输出**那些被判定为宏观事件，且 `importance_score >= 4` 的新闻条目！
    对于纯微观噪音、无效信息、或是分值低于4分的新闻，请**完全忽略，不要包含在输出数组中**！
    **如果所有传入的新闻中没有达到4分的，你应该尽可能选出那些最接近宏观的相对重要的新闻，并保守地给它们打分（例如4-5分），以确保我们不会得到一个完全为空的列表。**

    请直接返回一个严格的 JSON 数组，格式为：[{"id": 0, "importance_score": 8, "reason": "简短原因"}]
    如果实在没有任何新闻可以被勉强算作宏观，才允许返回空数组 []。
