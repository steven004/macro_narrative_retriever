...
def filter_news_with_llm(news_list, chunk_size=50):
    ...
    【极致重要：中国本土宏观的绝对保障机制（强制占比 >= 50%）】：
    我们的最终受众是中国投资者。因此在打分和筛选时，请强制保证最终输出的高分列表中，【中国国内宏观事件】（如中国央行、财政部、国内经济数据、A股重磅政策等）必须占据至少一半（50%）的绝对席位！
    哪怕某些海外新闻（如地缘冲突、外盘动荡）的绝对分数很高，只要它们加起来超过了一半的名额，都请你强制降低部分海外新闻的分数，强行把中国本土甚至稍显平淡的政策信息“顶”上来，确保最终报告中国内外视野不低于 1:1！
